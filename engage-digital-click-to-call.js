...
function screenShareController() {
  handleScreenShareStateChange(engageScreenShareState);
}
...
const screenShareState = {
  execute: () => {
    engageLog('Executing screensahre');
    screenShare();
  },
  name: 'screenSahre'
}

const screenUnshareState = {
  execute: () => {
    engageLog('Executing screenunsahre');
    screenUnshare();
...
let engageScreenShareState = screenShareState;

screenShareState.nextState = screenUnshareState;
screenUnshareState.nextState = screenShareState;
...
function screenShare() {
  if (engageDigitalSession && !isScreenSharing) {
    engageDigitalSession.screenShare();
  }
}

function screenUnshare() {
  if (engageDigitalSession && isScreenSharing) {
    engageDigitalSession.stopScreenShare();
  }
...
function handleScreenShareStateChange(screenState) {
  engageLog('Current state ' + engageScreenShareState.name);
  screenState.execute();
  engageScreenShareState = screenState.nextState;
  engageLog('Next state ' + engageScreenShareState.name);
}
